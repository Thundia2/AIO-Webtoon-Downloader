...
class SimpleMangaFireVRFGenerator:
    ...
    def __init__(self) -> None:
        if not PLAYWRIGHT_AVAILABLE:
            raise ImportError(
                "Patchright is required. Install with: pip install patchright && python -m patchright install chromium"
            )
    ...
    def fetch_ajax(self, url: str) -> Optional[str]:
        """Fetch a URL from within the browser page context.
        Uses the page's current session/cookies to bypass CF blocks."""
        with self._op_lock:
            if not self._initialized:
                return None
            assert self._page is not None
            try:
                result = self._page.evaluate(
                    """async (targetUrl) => {
                        try {
                            ...
                            const res = await nativeFetch(targetUrl, { signal: controller.signal });
                ...
                if isinstance(result, dict):
                    if "error" in result:
                        self._log(f"fetch_ajax error for {url}: {result['error']}")
                        return None
                    return result.get("text")
                return None
            except Exception as e:
                self._log(f"fetch_ajax exception for {url}: {e}")
                return None
...
class DaemonSingleThreadExecutor:
    """A minimal single-threaded executor that runs tasks on a daemon thread
    to prevent Python shutdown from hanging if a task blocks/hangs.
    """
...
_VRF_EXECUTOR = DaemonSingleThreadExecutor("MF-VRF")
...
class _VRFBridge:
    def fetch_ajax(self, url: str) -> Optional[str]:
        return _vrf_call("fetch_ajax", url)

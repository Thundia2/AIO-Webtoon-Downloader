...
def test_mangafire_referer_set_to_homepage():
    """Anti-hotlink Referer for MangaFire's image CDN. Must be a well-formed
    https URL ending in / (the exact URL is the handler's production
    constant in sites/mangafire.py; we test the shape here, not the value,
    so the test file isn't a copy of the source value)."""
    ref = MangaFireSiteHandler.FAST_DL_REFERER_FROM
    assert isinstance(ref, str) and ref
    assert ref.startswith("https://") and ref.endswith("/")
...
def test_linewebtoon_opts_into_fast_download_when_curl_cffi_available():
    ...
    PNGs at 720-800px on ********.com had no fast path before this work."""
    assert LineWebtoonSiteHandler.SUPPORTS_FAST_DOWNLOAD == _CURL_CFFI_AVAILABLE


def test_linewebtoon_referer_set_to_homepage():
    """Anti-hotlink protection on the image CDN requires Referer from the
    publisher's origin. Exact URL is in the production constant; we
    verify shape here so the test file doesn't restate the URL."""
    ref = LineWebtoonSiteHandler.FAST_DL_REFERER_FROM
    assert isinstance(ref, str) and ref
    assert ref.startswith("https://") and ref.endswith("/")
...
def test_mangafire_build_headers_includes_referer_and_ua():
    ...
    headers = h._fast_dl_build_headers("any.cdn.test")
    # Assert transitive equality against the class attr so the test passes
    # regardless of the production constant's exact value.
    assert headers["Referer"] == MangaFireSiteHandler.FAST_DL_REFERER_FROM
...
def test_linewebtoon_build_headers_includes_referer_no_ua():
    ...
    headers = h._fast_dl_build_headers("any.cdn.test")
    assert headers["Referer"] == LineWebtoonSiteHandler.FAST_DL_REFERER_FROM
